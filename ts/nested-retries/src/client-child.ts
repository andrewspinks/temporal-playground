...
const SCENARIOS: Array<{ name: string; description: string; request: TransferRequest }> = [
  ...
  },
  {
    ...
    description: 'Insufficient funds — daily retries, account topped up on day 3 so it succeeds',
    request: {
  ...
  },
  {
    ...
    description: 'Insufficient funds — daily retries for 14 days, never recovers, final failure SMS',
    request: {
...
async function run() {
  ...
  for (const scenario of SCENARIOS) {
    const workflowId = `transfer-child-${runId}-${scenario.name}`;
    await client.workflow.start(processTransfersWithChildren, {
  ...
  console.log(`Started ${SCENARIOS.length} workflows. Watch parent + child hierarchy in the Temporal UI.`);
}

...
const SCENARIOS: Array<{ name: string; description: string; request: TransferRequest }> = [
  ...
  },
  {
    ...
    description: 'Insufficient funds — Temporal retries daily, account topped up on day 3 so it succeeds',
    request: {
  ...
  },
  {
    ...
    description: 'Insufficient funds — Temporal retries for 14 days, never recovers, final failure SMS',
    request: {
...
async function run() {
  ...
  for (const scenario of SCENARIOS) {
    const workflowId = `transfer-child-retry-${runId}-${scenario.name}`;
    await client.workflow.start(processTransfersWithRetryPolicy, {
      taskQueue: 'payment-transfers-child-retry',
  ...
  console.log(`Started ${SCENARIOS.length} workflows. Each retry appears as a separate child run in the Temporal UI.`);
}

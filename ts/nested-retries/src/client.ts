...
const SCENARIOS: Array<{ name: string; description: string; request: TransferRequest }> = [
  ...
  },
  {
    ...
    request: {
      ...
      receivers: [{ ...RECEIVERS[0], simulateError: 'user-error' }, { ...RECEIVERS[1] }, { ...RECEIVERS[2] }],
    },
  },
  {
...
async function run() {
  ...
  for (const scenario of SCENARIOS) {
    const workflowId = `transfer-${runId}-${scenario.name}`;
  ...
  console.log(`Started ${SCENARIOS.length} workflows. Watch progress in the Temporal UI.`);
}

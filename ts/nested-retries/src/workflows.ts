...
// Payment: built-in retries handle only intermittent errors.
// Non-retryable and user errors bypass Temporal's retry and fall through to workflow logic.
...
// Notifications and manual review: simple retry for transient infra failures.
...
async function processReceiverTransfer(account: AccountInfo, receiver: ReceiverTransfer): Promise<TransferResult> {
  try {
    await processPayment(account.accountId, receiver.receiverId, receiver.amount, receiver.simulateError, receiver.succeedAfterAttempts, 1);
...
async function handleUserErrorWithDailyRetry(account: AccountInfo, receiver: ReceiverTransfer): Promise<TransferResult> {
  ...
  for (let day = 1; day <= MAX_DAILY_RETRIES; day++) {
    ...
    try {
      await processPayment(account.accountId, receiver.receiverId, receiver.amount, receiver.simulateError, receiver.succeedAfterAttempts, day + 1);
      return {
